...
namespace API.Controllers
{
    ...
    public class BaseApiController : ControllerBase
    {
        ...
        protected async Task<ActionResult> CreatePagedResult<T, TDto>(IGenericRepository<T> repo, ISpecification<T> spec, int pageIndex, int pageSize, Func<T, TDto> toDto) where T : BaseEntity, IDtoConertible
        {
            var items = await repo.ListAsync(spec);
            var count = await repo.CountAsync(spec);
